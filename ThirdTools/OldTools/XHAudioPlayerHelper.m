...
- (void)playAudioWithFileName:(NSString*)fileName {
    if (fileName.length > 0) {
        ...
        if (_playingFileName && [fileName isEqualToString:_playingFileName]) {//上次播放的录音
            ...
            NSString *wavFilePath = fileName;
            if([fileName containsString:@"http"]||[fileName containsString:@"www"])
            {
                ...
                {
                    amrFilePath = [NSString stringWithFormat:@"%@/%@.mp3", docDirPath , @"temp"];
                }
                else
                {
                    amrFilePath = [NSString stringWithFormat:@"%@/%@.amr", docDirPath , @"temp"];
                }
                
                BOOL Flag = [audioData writeToFile:amrFilePath atomically:YES];
                if(!Flag)
                {
                    return;
                }
                ...
                {
                    wavFilePath = [NSString stringWithFormat:@"%@/%@.wav", docDirPath , @"temp"];
                    int index = [VoiceConverter amrToWav:amrFilePath wavSavePath:wavFilePath];
                    if(index == 0)
                    {
                        return;
                    }
            ...
            AVAudioPlayer *pl = [[AVAudioPlayer alloc] initWithContentsOfURL:[NSURL fileURLWithPath:wavFilePath] error:nil];
            pl.delegate = self;
            [pl play];
            self.player = pl;
            [[UIDevice currentDevice] setProximityMonitoringEnabled:YES];
...
- (void)sensorStateChange:(NSNotificationCenter *)notification {
    ...
    if ([[UIDevice currentDevice] proximityState] == YES) {
        //黑屏
        MyLog(@"Device is close to user");
        [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayAndRecord error:nil];
        
    } else {
        //没黑屏幕
        MyLog(@"Device is not close to user");
        [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayback error:nil];
        if (!_player || !_player.isPlaying) {
