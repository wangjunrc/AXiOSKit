...
@objcMembers class AXShareBridge: NSObject {
    ...
    /// 分享
    func  share(type: AXShareTarget ,item:AXSocialShareContent, block:AXShareCompletionHandler? ){
      
        if type.isEqual(to: AXShareTargetWeiChatSession) {
        ...
        }else if type.isEqual(to: AXShareTargetWeiChatTimeLine){
            if !self.weChat.isInstalled() {
                block!(false,error(code: -2, msg: "未安装"))
            }else{
            self.weChat.share(type: .timeLine, item: item, block: block)
            }
        }else if type.isEqual(to: AXShareTargetSinaWeibo){
            if !self.weibo.isInstalled() {
                block!(false,error(code: -2, msg: "未安装"))
            }else{
            self.weibo.share(item: item, block: block)
            }
        }else if type.isEqual(to: AXShareTargetQQFriends){
