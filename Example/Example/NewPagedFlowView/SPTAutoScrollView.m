...
#import "SPTAutoScrollView.h"
#import "SPTPageControl.h"
#import "AXiOSTools.h"
#import "MSWeakTimer.h"


@interface SPTAutoScrollView ()<UIScrollViewDelegate>
...
@property (nonatomic, strong) SPTPageControl *pageControl;

@property (nonatomic, strong) MSWeakTimer * timer;

@end

@implementation SPTAutoScrollView
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _space = 4;
        _second = 3.0;
        _pageControlType = PSPageControlCentet;
...
- (void)reloadData{
    ...
    //   5 1 2 3 4 5 1
    self.imageArray = [NSMutableArray arrayWithArray:self.images];
    NSLog(@"self.imageArray>> %@",self.imageArray);
    
    [self.imageArray addObject:self.images[0]];
    [self.imageArray insertObject:self.images.lastObject atIndex:0];
    
    _scrollerView.frame = CGRectMake((SELF_WIDTH - _currentPageSize.width) / 2, 0, _currentPageSize.width, self.frame.size.height);
    _scrollerView.contentSize = CGSizeMake(_currentPageSize.width * self.imageArray.count, self.frame.size.height);
    
    for (NSInteger i = 0; i < self.imageArray.count; i++) {
        
        CGFloat imageY = (self.frame.size.height - self.currentPageSize.height)/2;
        
        UIView * view = [[UIView alloc] initWithFrame:CGRectMake(i * _currentPageSize.width, imageY, _currentPageSize.width, _currentPageSize.height)];
...
//        [imageView updateWithImageAtURL: imageName];
    ...
    [self updatePageConstrol];

}

- (void)updatePageConstrol {
    if (_images.count == 1) {
    ...
    switch (self.pageControlType) {
        case PSPageControlCentet:
        {
            self.pageControl.center = CGPointMake(self.center.x - 30.0, CGRectGetMinY(self.scrollerView.frame) + self.currentPageSize.height + 2.0);
        }
            break;
        case PSPageControlRight: {
            self.pageControl.origin = CGPointMake((self.frame.size.width - 90), CGRectGetMinY(self.scrollerView.frame) + self.currentPageSize.height);
        }
            break;
        default:
            break;
...
- (void)statrScroll:(CGFloat)second{
    if (_timer == nil && _isTimer) {
        _timer =  [[MSWeakTimer alloc]initWithTimeInterval:second target:self selector:@selector(autoNextPage) userInfo:nil repeats:YES dispatchQueue:nil];
    }
}
...
- (SPTPageControl *)pageControl {
    if (!_pageControl) {
        _pageControl = [[SPTPageControl alloc] init];
