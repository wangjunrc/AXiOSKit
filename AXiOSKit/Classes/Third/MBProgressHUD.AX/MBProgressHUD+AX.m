...
+ (MBProgressHUD *)ax_setupMBProgressHUDInView:(UIView *)view title:(NSString *)title details:(NSString *)details{
    if (view == nil) {
        view = AXBundleHelper.currentViewController.view;
    }
    // 快速显示一个提示信息
...
+ (void)ax_show:(NSString *)text icon:(NSString *)icon iconColor:(UIColor *)iconColor view:(UIView *)view {
    ...
    UIImage *image = nil;
    if (icon) {
        image =  [AXBundleHelper imageNamed:icon];
...
+ (void)ax_showAnimSuccess:(NSString *)title {
    [self ax_showAnimLoop:NO name:@"lottie_loading_success" title:title];
}

+ (void)ax_showAnimError:(NSString *)title {
    [self ax_showAnimLoop:NO name:@"lottie_loading_failure" title:title];
}

+ (void)ax_showAnimLoading:(NSString *)title {
    [self ax_showAnimLoop:YES name:@"lottie_loading" title:title];
}
...
+ (void)ax_showAnimLoop:(BOOL)loop
    ...
    LOTAnimationView *aniView = [LOTAnimationView animationNamed:name inBundle:[AXBundleHelper bundleWithName:@"AXLottie"]];
    
    aniView.contentMode = UIViewContentModeScaleAspectFill;
    aniView.loopAnimation = loop;
    [aniView play];
    
    MBProgressHUD *hud =  [self ax_setupMBProgressHUDInView:nil title:title];
    // 设置图片
